...
func TestImageProcessor(t *testing.T) {
	// 创建图片处理器
	config := &wordprocessingml.ImageProcessorConfig{
		MaxImages:        100,
	...
	ip := wordprocessingml.NewImageProcessor(config)
	...
	if image.Format != wordprocessingml.ImageProcessorImageFormatJPEG {
		t.Errorf("图片格式不匹配，期望: jpeg，实际: %s", image.Format)
	}
	
	// 测试插入图片
	position := &wordprocessingml.ImageProcessorPosition{
		X:         100,
		Y:         200,
		ZIndex:    1,
		Alignment: wordprocessingml.ImageProcessorAlignmentCenter,
		Wrapping:  wordprocessingml.ImageProcessorWrappingSquare,
	}
	...
	// 测试调整图片大小
	size := &wordprocessingml.ImageProcessorSize{
		Width:              400,
		Height:             300,
		ScaleX:             0.8,
		ScaleY:             0.8,
...
func TestImageProcessorErrorHandling(t *testing.T) {
	ip := wordprocessingml.NewImageProcessor(nil)
