...
func TestRevisionTracker(t *testing.T) {
	// 创建修订跟踪器
	config := &wordprocessingml.RevisionTrackerConfig{
		MaxRevisions:      100,
		MaxComments:       50,
		MaxSuggestions:    20,
		AutoCleanup:       false,
	...
	rt := wordprocessingml.NewRevisionTracker(config)
	
	// 测试跟踪修订
	revision := &wordprocessingml.RevisionTrackerRevision{
		Type:        wordprocessingml.RevisionTrackerChangeTypeInsert,
		Content:     "测试内容",
		Author:      "test_user",
		Description: "测试修订",
	}
	...
	// 测试添加评论
	comment := &wordprocessingml.RevisionTrackerComment{
		Content: "测试评论",
		Author:  "test_user",
	}
...
func TestCollaborativeEditor(t *testing.T) {
	// 创建修订跟踪器
	revisionTracker := wordprocessingml.NewRevisionTracker(nil)
	
	// 创建协作编辑器
	config := &wordprocessingml.CollaborativeEditorConfig{
		MaxSessions:       10,
		MaxOperations:     100,
		MaxUsers:          5,
		AutoCleanup:       false,
	...
	ce := wordprocessingml.NewCollaborativeEditor(config, revisionTracker)
	
	// 创建用户
	creator := &wordprocessingml.CollaborativeEditorUser{
		ID:     "user1",
		Name:   "测试用户1",
		Email:  "user1@test.com",
		Role:   wordprocessingml.CollaborativeEditorUserRoleOwner,
		Status: wordprocessingml.CollaborativeEditorUserStatusActive,
	}
	...
	// 测试应用操作
	operation := &wordprocessingml.CollaborativeEditorOperation{
		UserID:  "user1",
		Type:    wordprocessingml.CollaborativeEditorOperationTypeInsert,
		Content: "插入的内容",
		Position: &wordprocessingml.CollaborativeEditorPosition{
			Start: 0,
			End:   10,
		},
	}
...
func TestDiscussionManager(t *testing.T) {
	...
	// 创建讨论管理器
	config := &wordprocessingml.DiscussionManagerConfig{
		MaxDiscussions:    50,
		MaxComments:       100,
		MaxThreads:        10,
		MaxNotifications:  50,
	...
	dm := wordprocessingml.NewDiscussionManager(config, revisionTracker)
	
	// 创建讨论线程
	thread := &wordprocessingml.DiscussionManagerThread{
		Title:       "测试线程",
		Description: "这是一个测试线程",
		Creator:     "user1",
		Type:        wordprocessingml.DiscussionManagerThreadTypeGeneral,
	}
	...
	// 创建讨论
	discussion := &wordprocessingml.DiscussionManagerDiscussion{
		Title:    "测试讨论",
		Content:  "这是一个测试讨论的内容",
		Author:   "user1",
		ThreadID: thread.ID,
		Type:     wordprocessingml.DiscussionManagerDiscussionTypeQuestion,
		Priority: wordprocessingml.DiscussionManagerPriorityMedium,
	}
