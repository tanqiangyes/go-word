...
func demoBatchProcessing() {
    ...
    // 创建批量处理器
    processor := word.NewBatchProcessor(3) // 3个并发
    ...
    // 添加操作
    processor.AddOperation(word.BatchOperation{
...
func createTestDocuments(count int) []*word.Document {
    var documents []*word.Document
...
func createProblematicDocument() *word.Document {
    // 创建文档写入器
    w := writer.NewDocumentWriter()
    w.CreateNewDocument()
    ...
    // 打开文档
    doc, err := word.Open("problematic_doc.docx")
    if err != nil {
...
func createLargeDocument() *word.Document {
    // 创建文档写入器
    w := writer.NewDocumentWriter()
    w.CreateNewDocument()
    ...
    // 打开文档
    doc, err := word.Open("large_doc.docx")
    if err != nil {
...
func monitorProgress(processor *word.BatchProcessor) {
...
// monitorErrors 监控错误
func monitorErrors(processor *word.BatchProcessor) {
