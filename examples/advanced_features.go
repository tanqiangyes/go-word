...
func demoDocumentStructure() {
    ...
    // 打开文档并分析结构
    doc, err := word.Open("structure_demo.docx")
    if err != nil {
...
func demoDocumentMerge() {
    ...
    // 打开目标文档
    targetDoc, err := word.Open("doc1.docx")
    if err != nil {
    ...
    // 打开源文档
    sourceDoc, err := word.Open("doc2.docx")
    if err != nil {
    ...
    // 创建合并操作
    merge := word.NewDocumentMerge(targetDoc)
    merge.AddSourceDocument(sourceDoc)

    // 设置合并选项
    options := word.MergeOptions{
        MergeMode:          word.AppendMode,
        ConflictResolution: word.KeepFirst,
...
func demoTemplateProcessing() {
    ...
    // 打开模板文档
    doc, err := word.Open("template.docx")
    if err != nil {
    ...
    // 添加自定义占位符
    placeholder := word.TemplatePlaceholder{
        Key:          "title",
        Type:         word.TextPlaceholder,
        Required:     true,
        DefaultValue: "默认标题",
    }
