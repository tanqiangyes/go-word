...
func demoAdvancedText() {
    ...
    // 创建高级文本系统
    textSystem := word.NewAdvancedTextSystem()
...
最后，这个文档展示了高级文本处理功能。`
    ...
    // 应用发光效果
    glowProperties := &word.EffectProperties{
        Color:    "FFFF00",
        Size:     5.0,
        Opacity:  0.8,
        X:        0.0,
    ...
    if err := textSystem.ApplyTextEffect(text.ID, word.GlowEffect, glowProperties); err != nil {
    ...
    // 应用阴影效果
    shadowProperties := &word.EffectProperties{
        Color:    "808080",
        Size:     3.0,
        Opacity:  0.6,
        X:        2.0,
    ...
    if err := textSystem.ApplyTextEffect(text.ID, word.ShadowEffect, shadowProperties); err != nil {
