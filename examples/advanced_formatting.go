...
func demoComplexTable() {
	...
	// 创建高级格式化器
	doc, err := wordprocessingml.Open("temp_doc.docx")
	if err != nil {
	...
	// 设置单元格边框
	borders := wordprocessingml.CellBorders{
		Top:    wordprocessingml.BorderSide{Style: "double", Size: 2, Color: "000000"},
	...
	// 设置单元格底纹
	shading := wordprocessingml.CellShading{
		Fill:  "solid",
		Color: "CCCCCC",
		Val:   "clear",
	}
...
func demoHeaderFooter() {
	...
	// 创建页眉
	header := formatter.CreateHeader(wordprocessingml.HeaderType)
	...
	// 创建页脚
	footer := formatter.CreateFooter(wordprocessingml.FooterType)
	footer.Content[0].Text = "页码: 第 1 页"
	footer.Content[0].Runs[0].Text = "页码: 第 1 页"
	footer.Content[0].Runs[0].FontSize = 10

	// 创建首页页眉
	firstHeader := formatter.CreateHeader(wordprocessingml.FirstHeaderType)
	...
	// 创建首页页脚
	firstFooter := formatter.CreateFooter(wordprocessingml.FirstFooterType)
...
func demoPageAndSection() {
	...
	section.Content = []wordprocessingml.Paragraph{
		{
			Text:  "第二节内容",
			Style: "Heading2",
