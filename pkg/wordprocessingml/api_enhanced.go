...
func (b *EnhancedDocumentBuilder) SaveDocument(doc *Document, filepath string) error {
	...
	// 配置保存选项
	saveOptions := &types.SaveOptions{
		FilePath:    filepath,
		Format:      types.DocumentFormatDocx,
		Compression: true,
		Backup:      b.config.Backup.Enabled,
		BackupPath:  b.config.Backup.Path,
		Metadata:    true,
		Properties:  true,
	}

	// 保存文档
	if err := writer.SaveDocument(doc, saveOptions); err != nil {
	...
	b.logger.Info("文档已保存", map[string]interface{}{
		...
		"backup_enabled": b.config.Backup.Enabled,
	})
...
func (b *EnhancedDocumentBuilder) exportToPDF(doc *Document, filepath string) error {
	// 创建PDF导出器
	pdfExporter := NewPDFExporter(doc)

	// 配置导出选项
	exportConfig := &types.PDFExportConfig{
		Quality:     types.PDFQualityHigh,
		Compression: true,
		Margins: &types.PDFMargins{
			Top:    20,
			Bottom: 20,
			Left:   25,
			Right:  25,
	...
	// 导出PDF
	if err := pdfExporter.ExportToPDF(context.Background(), filepath); err != nil {
...
func (b *EnhancedDocumentBuilder) exportToRTF(doc *Document, filepath string) error {
	...
	// 转换为RTF格式
	if err := formatSupport.ConvertFormat(RtfFormat); err != nil {
		return fmt.Errorf("RTF转换失败: %w", err)
	}

	b.logger.Info("文档已导出为RTF", map[string]interface{}{
		"filepath": filepath,
	})
...
func (b *EnhancedDocumentBuilder) GetDocumentStatistics(doc *Document) (*types.DocumentStatistics, error) {
	...
	// 统计段落数量
	if paragraphs, err := doc.GetParagraphs(); err == nil {
		stats.ParagraphCount = len(paragraphs)
		stats.WordCount = b.countWords(paragraphs)
		stats.CharacterCount = b.countCharacters(paragraphs)
	}

	// 统计表格数量
	if tables, err := doc.GetTables(); err == nil {
		stats.TableCount = len(tables)
		stats.CellCount = b.countCells(tables)
	}

	// 统计图片数量
	if images, err := doc.GetImages(); err == nil {
		stats.ImageCount = len(images)
	}

	// 统计样式数量
	if styles, err := doc.GetStyles(); err == nil {
		stats.StyleCount = len(styles)
	}
