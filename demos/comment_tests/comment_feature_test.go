...
func main() {
	...
	// 为表格添加批注
	err = docWriter.AddComment("项目经理", "这个项目进度表需要每周更新，确保所有团队成员都能看到最新状态。", "项目进度表")
	if err != nil {
		log.Fatal("Failed to add comment to table:", err)
	}
	fmt.Println("✅ 为表格添加批注成功")
	...
	err = docWriter.AddParagraph("批注功能测试总结：本次测试验证了批注功能在多种场景下的表现，包括基本段落、表格、长文本等。", "Normal")
	if err != nil {
