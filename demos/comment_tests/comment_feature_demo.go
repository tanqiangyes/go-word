...
func main() {
	...
	err = docWriter.AddParagraph("批注功能测试文档", "Normal")
	if err != nil {
	...
	// 测试5: 添加更多批注测试
	fmt.Println("5. 添加更多批注测试...")
	
	// 添加一个简单的段落
	err = docWriter.AddParagraph("这是一个测试段落，用于验证批注功能的稳定性。", "Normal")
	if err != nil {
		log.Fatal("Failed to add test paragraph:", err)
	}
	
	// 为测试段落添加批注
	err = docWriter.AddComment("测试员", "批注功能测试通过，功能正常。", "这是一个测试段落，用于验证批注功能的稳定性。")
	if err != nil {
		log.Fatal("Failed to add comment to test paragraph:", err)
	}
	fmt.Println("✅ 为测试段落添加批注成功")

	// 测试6: 添加总结段落
	fmt.Println("6. 添加总结段落...")
	err = docWriter.AddParagraph("批注功能测试总结：本次测试验证了批注功能在多种场景下的表现，包括基本段落、长文本等。", "Normal")
	if err != nil {
		log.Fatal("Failed to add summary paragraph:", err)
	}

	// 保存文档
	filename := "comment_feature_test.docx"
